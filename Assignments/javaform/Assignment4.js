/*
      ...
      Designed by: Chandra K. Rai & Nishan Basu
      Student ID No: 300890510 & 300915261 
...
*/
    ...
    try {
        ...
        if (/\d/.test(firstName.value) || /^\s*$/.test(firstName.value)) {
            fieldSetValidity = false;
            errorDict[firstName.id] = "(Only characters allowed)";
        }
        ...
        if (/\d/.test(lastName.value) || /^\s*$/.test(lastName.value)) {
            fieldSetValidity = false;
            errorDict[lastName.id] = "(Only characters allowed)";
        } 
    ...
    try {
        ...
        if (/\d/.test(city.value) || /^\s*$/.test(city.value)) {
            fieldSetValidity = false;
            errorDict[city.id] = "(Only characters allowed)";
        }
        //validate #street
        if (/^\s*$/.test(street.value)) {
            fieldSetValidity = false;
            errorDict[street.id] = "(Only characters allowed)";
        }
        //validate #postalcode
    ...
    try {
        ...
        //validate #previouseduc
        if (previouseduc.value === '') {
            fieldSetValidity = false;
            errorDict[previouseduc.id] = "(Please select previous education)";
        }
